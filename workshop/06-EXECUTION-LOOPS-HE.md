...
### Goal, Plan ורשימת המשימות אינם אותו דבר

```text
GOAL (יציב) → SPEC (מה ולמה) → PLAN (איך ובאיזה סדר) → TASK LIST (המסלול כרגע) → EVIDENCE
```

- `GOAL.md` מגדיר את התוצאה, ה-non-goals, הקריטריונים ותנאי העצירה. הוא לא משתנה רק מפני שהביצוע הסתבך.
- `FEATURE_SPEC.md` מגדיר את התנהגות המוצר, ה-states וה-acceptance criteria. מאשרים אותו לפני planning.
- `PLAN.md` שומר את החלטות הארכיטקטורה, הקבצים, ה-contracts והבדיקות.
- רשימת המשימות היא working memory קצר: מה ממתין, מה מתבצע, מה הושלם ומה נחסם. מעדכנים אותה כשהכלי מחזיר מידע חדש.
- משימה מסומנת `complete` רק אחרי פעולה וראיה מתאימה. כתיבת קוד לבדה אינה הוכחת סיום.

אם רשימת המשימות מתנגשת ב-Goal, לא משנים את ה-Goal בשקט. חוזרים ל-Plan, או עוצרים להחלטה אנושית.
...
ה-Agent הראשי עושה fan-in: מסיר כפילויות, פותר סתירות, מתעד חלופות שנדחו ומנסח Spec או verdict אחד. המשתתף מאשר החלטות מוצר; לא מחברים memos ולא מעבירים ל-Agent את הבעלות על הכיוון.
...
אותו שם effort לא מייצג את אותו reasoning budget בכל מודל. התמיכה ברמות משתנה לפי מודל, חשבון וגרסה; בדקו `/model` ו-`/usage` בבוקר הסדנה. `max` הוא session-only, אלא אם הוגדר דרך משתנה סביבה. המילה `ultrathink` מבקשת עומק נוסף לפנייה אחת, אבל לא משנה את רמת ה-effort שנשלחת ל-API.
